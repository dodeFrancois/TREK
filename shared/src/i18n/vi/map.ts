import type { TranslationStrings } from '../types';

const map: TranslationStrings = {
  'map.connections': 'Kết nối',
  'map.showConnections': 'Hiển thị lộ trình đặt vé',
  'map.hideConnections': 'Ẩn lộ trình đặt chỗ',
  'map.search.placeholder': 'Tìm địa điểm trên bản đồ',
  'map.search.failed': 'Không thể tìm kiếm — bộ mã hóa địa lý không phản hồi',
  'map.search.addToTrip': 'Thêm vào chuyến đi',
  'poi.searchThisArea': 'Tìm kiếm khu vực này',
  'poi.cat.restaurants': 'Nhà hàng',
  'poi.cat.cafes': 'Quán cà phê',
  'poi.cat.bars': 'Quán bar & cuộc sống về đêm',
  'poi.cat.hotels': 'Chỗ ở',
  'poi.cat.sights': 'Điểm tham quan',
  'poi.cat.museums': 'Bảo tàng & văn hóa',
  'poi.cat.nature': 'Thiên nhiên & công viên',
  'poi.cat.activities': 'Các hoạt động',
  'map.showAllConnections': 'Hiển thị tất cả lộ trình đặt chỗ',
  'map.hideAllConnections': 'Ẩn tất cả lộ trình đặt chỗ',
};
export default map;
